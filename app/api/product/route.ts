...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!b.tokenId || !b.owner || !b.metadataCid) {
    ...
    const product = await prisma.product.create({
      data: {
        ...
        name: b.name,
        description: b.description, // <-- ini harus pasti ada
        imageCid: b.imageCid?.replace(/^ipfs:\/\//, '') ?? '',
...
export async function PUT(req: NextRequest) {
  try {
    ...
    if (typeof b.quantity === "undefined")
      return NextResponse.json({ error: "quantity required" }, { status: 400 });

    // Optional: server-side verify owner via signed message
